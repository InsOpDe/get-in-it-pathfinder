...
import {convert, ESolver, IImportGraph, IProblem, ISolution, solve, TGraphLabelMap} from "./internal";
...
const printHelp = () => {
	...
	console.log("node app.js --file graph.json --start erde --end b3-r7-r4nd7 --algorithm 0");
};
const init = () => {
	console.log(
		chalk.default.green(
...
const askNode = async (labelMap: TGraphLabelMap, nodetype: string, label?: string) => {
	if(!label) {
		const question = [{
			name: "label",
			type: "input",
	...
	if(typeof node === "undefined") {
		console.log(chalk.default.red.bold(`The ${nodetype} node ${label} can't be found`));
...
const success = (solution: ISolution, time: number): void => {
	console.log(
		chalk.default.white.bgGreen.bold(`Done in ${time}ms! Path is ${solution.path.join(", ")} with an overall cost of ${solution.cost}`)
	);
};
const selectAlgorithm = (selection: string): ESolver => {
	switch (selection.toLowerCase().trim().replace(" ","")) {
		case "0":
		case "bellmanford":
		default:
